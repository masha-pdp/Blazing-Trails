...
export function initialize(hostElement, routeMapComponent, existingWaypoints, isReadOnly) {
    ...
    hostElement.map.on('click', function (e) {
        let waypoint = L.marker(e.latlng); waypoint.addTo(hostElement.map); hostElement.waypoints.push(waypoint);
    ...
    if (!isReadOnly){
        hostElement.map.on('click', function (e){
            ...
            let line = L.polyline(hostElement.waypoints
                ...
                .addTo(hostElement.map);
            hostElement.lines.push(line);
            outeMapComponent.invokeMethodAsync('WaypointAdded',e.latlng.lat, e.latlng.lng);
        });
    }
}
...
export function deleteLastWaypoint(hostElement) {
    if (hostElement.waypoints.length > 0) {
        ...
        if (hostElement.lines.length > 0) {
            ...
            // return `Deleted waypoint at latitude ${lastWaypoint.getLatLng().lat} 
            //                             longitude ${lastWaypoint.getLatLng().lng}`;
